...
def visualize_boxes_and_labels_on_image_array(
  ...
  # Sorting Labels From Left to Right
  sort = {}
  for char, xmin in zip(labels, x_coordinate):
    sort[char] = xmin
  
  sorting = {k: v for k, v in sorted(sort.items(), key=lambda item: item[1])}
  final_labels = list(sorting.keys())

  return final_labels, image
